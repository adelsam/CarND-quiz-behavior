...
public:
    ...
    */
    ...
    vector<string> compute_possible_states();

    double calc_lane_cost(string state);

    double calc_speed_cost(string state);

    double calc_on_road_cost(string state);

    double calc_avoid_other_cars(string state, map<int, vector<vector<int> > > predictions);
};
